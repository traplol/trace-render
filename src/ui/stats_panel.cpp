...
void StatsPanel::render_tab(QueryTab& tab, const TraceModel& model, QueryDb& db, ViewState& view) {
    ...
    float results_h = ImGui::GetContentRegionAvail().y - ImGui::GetFrameHeightWithSpacing();
                ...
                for (int c = 0; c < col_count && c < (int)row.size(); c++) {
                    ...
                    if (c == name_col) {
                        ...
                        if (ImGui::Selectable(
                            ...
                            // Find first event with this name via pre-computed index
                            const std::string& name = row[name_col];
                            auto sm_it = model.string_map().find(name);
                            if (sm_it != model.string_map().end()) {
                                auto ne_it = model.name_to_events().find(sm_it->second);
                                if (ne_it != model.name_to_events().end() && !ne_it->second.empty()) {
                                    uint32_t ei = ne_it->second[0];
                                    view.navigate_to_event(ei, model.events()[ei]);
                                }
                            }
                        }
                        ImGui::SameLine();
