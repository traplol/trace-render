...
void FlowRenderer::render(ImDrawList* dl, const TraceModel& model, const ViewState& view,
    ...
    dl->PushClipRect(ImVec2(track_left, area_min.y), area_max, true);
    ...
    for (const auto& [id, indices] : model.flow_groups_) {
        ...
        for (size_t i = 0; i + 1 < indices.size(); i++) {
            ...
            if (ev1.ts > view.view_end_ts && ev2.ts > view.view_end_ts)
                continue;
            if (ev1.end_ts() < view.view_start_ts && ev2.end_ts() < view.view_start_ts)
                continue;
            ...
            if (it1 == track_positions_.end() || it2 == track_positions_.end())
                continue;
            ...
            ImVec2 a1(p2.x - arrow_size * std::cos(angle - 0.4f),
                      p2.y - arrow_size * std::sin(angle - 0.4f));
            ImVec2 a2(p2.x - arrow_size * std::cos(angle + 0.4f),
                      p2.y - arrow_size * std::sin(angle + 0.4f));
